...
export const SideMenuComponent: React.FC<SideMenuProps> = ({
  ...
  return (
    <div onClick={() => setOpen(!open)} className={cn(styles.wrap,{[styles.wrap_hide]: open})}>
      <div className={cn(styles.contentContainer,{[styles.contentContainer_hide]: open})}>
        <header className={cn(styles.header,{[styles.header_hide]: open})}>
            {/*{*/}
            {/*  open ?*/}
      ...
      <footer>
        <div className={cn(styles.social,{[styles.social_hidden]: open})}>
            {
              socialArr.map((el,i) =>
              <div key={i} className={styles.ico}> <SvgIcon src={IconsEnum[el]} />
                <span className={cn(styles.user,{[styles.hide]: open})}>{el}</span>
              </div>)
            }
          <div className={styles.rights}>
            <span>The Cage 2022</span>Ⓒ<span>All rights reserved</span>
          </div>
        </div>
      </footer>
    </div>
